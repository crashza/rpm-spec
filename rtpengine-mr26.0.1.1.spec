...
Source0:    https://github.com/sipwise/rtpengine/archive/refs/tags/%{version}.tar.gz#/%{name}-%{version}.tar.gz
...
BuildRequires:	pkgconfig(opus) libjwt-devel
%if 0%{?rhel} == 8
...
sed -i -e "s/PACKAGE_NAME=\"ngcp-rtpengine\"/PACKAGE_NAME=\"rtpengine\"/g" %{buildroot}%{_usrsrc}/%{name}-%{version}-%{release}/dkms.conf
%if 0%{?with_transcoding} > 0
...
%changelog
* Tue Apr 21 2026 Trevor Steyn <trevor@webon.co.za>
  - updated for mr26
* Tue Jul 15 2025 Trevor Steyn <trevor@webon.co.za>
  - force g729 full support
